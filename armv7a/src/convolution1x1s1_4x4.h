...
void conv1x1s1SgemmTransformKenel(float *const &kernel, float* &dest, const int &inChannel, const int &outChannel){
        int c = 0;
...
void conv1x1s1SgemmTransformInput(float *const &src, const int &inWidth, const int &inHeight,  const int &inChannel, float* &src_tm,
    ...
    // pack input start
    int nnSize = outSize >> 2;
    int remainSize = 0;

    int src_tm_channel = outSize / 4 + outSize % 4;
    int src_tm_h = 4 * 4;
    ...
    for(int i = 0; i < nnSize; i++){
        ...
        float *src_tm_ptr = src_tm + (newi / 4) * src_tm_size;
    ...
    for(int i = remainSize; i < outSize; i++){
        ...
        float *src_tm_ptr = src_tm + (newi / 4 + newi % 4) * src_tm_size;
...
void conv1x1s1SgemmNeon(float *const &src, float *const &src_tm ,const int &inWidth, const int &inHeight,  const int &inChannel, float *const &kernel,
    ...
    #pragma omp parallel for num_threads(OMP_THREAD)
        ...
        for(int cc = 0; cc < nnOutChannel; cc++){
            ...
            for(; i < outSize; i++){
                const float *src_tm_ptr = src_tm + (i / 4 + i % 4) * src_tm_size;
                const float *kernel0 = kernel + (c / 4) *  kernelSize;
